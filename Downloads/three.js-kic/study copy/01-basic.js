...
// import eye_To_camera from "./pnp2.js";

// console.log(eye_To_camera, "webgl-container");
...
class App {
	...
	_setupModel() {
		...
		const material = new THREE.MeshPhongMaterial({ color: "#353535" });
	...
	update(time) {
		...
		this._camera.position.x = -(dao_x.textContent * 2).toFixed(10);
		this._camera.position.y = -(dao_y.textContent * 2).toFixed(10);
		// this._camera.position.x = -(dao_x.textContent * 2);
		// this._camera.position.y = -(dao_y.textContent * 2);
		console.log(Number(-dao_x.textContent), "dao");
